...
impl LiveActor {
    ...
    async fn run_inner(&mut self) -> Result<oneshot::Sender<()>> {
        ...
        loop {
            ...
            // Cargo.toml): upstream used `biased;` here, which always services
            // the inbox before draining `replica_events_rx`. Inbox handlers
            // (IncomingSyncReport → has_news_for_us, StartSync → open) call
            // back into the single-threaded sync actor and await its reply —
            // and under a fleet-scale sync-report storm the sync actor is
            // itself blocked sending into `replica_events` (see the matching
            // patch in sync.rs). Biased polling made that priority inversion
            // permanent: the drain arm was never reached. Fair polling lets
            // events drain regardless of inbox pressure.
            tokio::select! {
                msg = self.inbox.recv() => {
