...
# `systemd --user` service under a per-user data dir (~/.local/share/seedsync).
# A root-run scriptlet has no reliable way to see every desktop account's own
# tarball install (`~/.local/bin/seed-daemon`, ahead of /usr/bin on most
# desktops' PATH), so there is nothing safe to refuse here. The conflict is
